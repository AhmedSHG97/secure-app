...
update_option( 'wbc907_theme_token', 'token' );
set_transient( 'wbc907_theme_token', 'token', 0 );



if ( defined( 'WBC907_CORE_PLUGIN_VERSION' ) ) {
	set_transient( 'wbc907_theme_plugin_token'.str_replace('.','_',WBC907_CORE_PLUGIN_VERSION), 'token', 0 );
}

add_filter( 'wbc907_theme_plugins_filter', function( $plugins ){
	foreach ( $plugins as $key => $plugin ) {
		switch ( $plugin['slug'] ) {
			case 'wbc907-core':
				$plugins[ $key ]['source'] = trailingslashit(get_template_directory_uri()). 'plugins/wbc907-core-plugin.zip';
				break;
			case 'revslider':
				$plugins[ $key ]['source'] = trailingslashit(get_template_directory_uri()). 'plugins/revslider.zip';
				break;
			case 'js_composer':
				$plugins[ $key ]['source'] = trailingslashit(get_template_directory_uri()). 'plugins/js_composer.zip';
				break;
		}
	}
	return $plugins;
...
if ( !defined( 'WBC907THEME_VERSION' ) ) {
	define( 'WBC907THEME_VERSION', '5.1.8' );
}
