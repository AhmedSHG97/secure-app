...
if ( ! function_exists( 'towy_google_font_url' ) ) :
	function towy_google_font_url() {
		$fonts_url = '';
		$fonts     = array();
		...
		$fonts['Roboto'] = array(
			...
			'subset'         => 'latin-ext',
			'variation'      => '300',
			'variants'       => array( '300', '300italic', 'regular', '700', '700italic' ),
			'family'         => 'Roboto',
			'style'          => false,
			'weight'         => false,
			'size'           => '14',
...
if ( ! function_exists( 'towy_add_font_styles_in_head' ) ) :
	function towy_add_font_styles_in_head() {
...
if ( ! function_exists( 'towy_return_comments_walker' ) ) :
	function towy_return_comments_walker() {
		return new Towy_Comments_Walker;
	}
endif;


if ( ! function_exists( 'towy_the_attached_image' ) ) :
	/**
	 * Print the attached image with a link to the next attached image.
	 */
	function towy_the_attached_image() {
		$post = get_post();
		/**
		 ...
		 * }
		 */
		$attachment_size     = apply_filters( 'towy_attachment_size', array( 810, 810 ) );
	...
	} //towy_the_attached_image()

endif;

if ( ! function_exists( 'towy_list_authors' ) ) :
	/**
	 * Print a list of all site authors who published at least one post.
	 */
	function towy_list_authors() {
		...
		foreach ( $author_ids as $author_id ) :
			...
			?>
			<div class="author-meta side-item content-padding with_background cs">
				<div class="row">
					<div class="col-md-4">
					...
					<div class="col-md-8">
						<div class="item-content">
							...
							if ( $twitter_url || $facebook_url || $google_plus_url ) :
								?>
								<span class="author-social darklinks">
									<?php if ( $twitter_url ) : ?>
...
if ( ! function_exists( 'towy_get_columns_classes' ) ) :
	/**
	 * Define a sidebar position for manage main column CSS class, sidebar CSS class and visibility of sidebar.
	 * return array
	 */
	function towy_get_columns_classes( $full_width = false ) {

        //default values
	    $position = 'right';
        if ( is_page() ) { $position = 'full'; }

        if ( function_exists( 'fw_ext_sidebars_get_current_position' ) ) {

            if ( in_array( fw_ext_sidebars_get_current_position(), array( 'full' ) ) ) { $position = 'full'; }

            elseif ( in_array( fw_ext_sidebars_get_current_position(), array( 'left' ) ) ) { $position = 'left'; }

            elseif ( in_array( fw_ext_sidebars_get_current_position(), array( 'right' ) ) ) { $position = 'right'; }
        ...
        if ( $full_width ) { $position = 'full'; }

        // Content/Sidebar width

        $s = 4;             // sidebar width

        if ( towy_is_shop() || towy_is_woocommerce_page() ) { $s = 3; }

        $s = apply_filters( 'towy_sidebar_width', $s );
        ...
        $column_classes = array(
            'main_column_class' => $content_column . 'col-sm-7 col-md-' . $c . ' col-lg-' . $c,
            'sidebar_class'     => $sidebar_column . 'col-sm-5 col-md-' . $s . ' col-lg-' . $s
        );

        if ( 'left' == $position ) {
            $column_classes['main_column_class'] = $content_column . "col-sm-7 col-md-" . $c . " col-lg-" . $c . " col-sm-push-5 col-md-push-" . $s . " col-lg-push-" . $s;
            $column_classes['sidebar_class']     = $sidebar_column . "col-sm-5 col-md-" . $s . " col-lg-" . $s . " col-sm-pull-7 col-md-pull-" . $c . " col-lg-pull-" . $c;
        }

        if ( 'full' == $position ) {
            $column_classes['main_column_class'] = $content_column . "col-sm-12";
            $column_classes['sidebar_class']     = false;
        }

		return $column_classes;

	} //towy_get_columns_classes()
 ...
 */
if ( ! function_exists( 'towy_bootstrap_paginate_links' ) ) {
	function towy_bootstrap_paginate_links( $args = '' ) {
		...
		$defaults = array(
			...
			'show_all'           => false,
			'prev_next'          => true,
			'prev_text'          => esc_html__( 'Prev', 'towy' ),
			'next_text'          => esc_html__( 'Next', 'towy' ),
			'end_size'           => 1,
			'mid_size'           => 2,
			'type'               => 'plain',
			'add_args'           => array(),
		...
		return $r;
	} //towy_bootstrap_paginate_links()
}

if ( ! function_exists( 'towy_paging_nav' ) ) :
	/**
	 * Display navigation to next/previous set of posts when applicable.
	 */
	function towy_paging_nav( $wp_query = null, $wrapper = null ) {
		...
		$links = towy_bootstrap_paginate_links( array(
			'base'      => $pagenum_link,
			'format'    => $format,
			'show_all'  => false,
...
if ( ! function_exists( 'towy_paging_comments_nav ' ) ) :
	/**
	 * Display navigation to next/previous set of posts when applicable.
	 */
	function towy_paging_comments_nav( $args = array() ) {
		...
		$args       = wp_parse_args( $args, $defaults );
		$page_links = towy_bootstrap_paginate_links( $args );
 ...
 */
if ( ! function_exists( 'towy_categorized_blog' ) ) :
	function towy_categorized_blog() {
		if ( false === ( $all_categories = get_transient( 'towy_category_count' ) ) ) {
			...
			set_transient( 'towy_category_count', $all_categories );
		}

		if ( 1 !== (int) $all_categories ) {
			// This blog has more than 1 category so towy_categorized_blog should return true
			return true;
		} else {
			// This blog has only 1 category so towy_categorized_blog should return false
			return false;
		}
	} //towy_categorized_blog()
endif;

if ( ! function_exists( 'towy_post_nav' ) ) :
	/**
	 * Display navigation to next/previous post when applicable.
	 */
	function towy_post_nav() {
		...
		<nav class="navigation post-navigation items-nav greylinks" role="navigation">
			...
			?>
			<?php previous_post_link( '%link', '<div class="media with_background  prev-item text-center" style="background-image: url(' . esc_url( $prev_thumbnail ) . '); "><div class="nav-overlay"></div><div class="nav-middle"><span class="nav">' . esc_html__( 'Prev', 'towy' ) . '</span><span class="title">%title</span></div></div>' ); ?>
			<?php next_post_link( '%link', '<div class="media with_background next-item text-center" style="background-image: url(' . esc_url( $next_thumbnail ) . '); "><div class="nav-overlay"></div><div class="nav-middle"><span class="nav">' . esc_html__( 'Next', 'towy' ) . '</span><span class="title">%title</span></div></div>' ); ?>
		</nav><!-- .navigation -->
	<?php } //towy_post_nav
endif;
...
endif; //towy_posted_on

if ( ! function_exists( 'towy_posted_by' ) ) : /**
 * Print HTML with meta information for the current post-date/time and author.
 */
	function towy_posted_by() {
		if ( is_sticky() && is_home() && ! is_paged() ) {
			echo '<span class="featured-post"><i class="rt-icon2-clip highlight"></i>' . esc_html__( ' Sticky: ', 'towy' ) . '</span>';
		}
		...
		/* translators: %s: post author */
			'<span class="author vcard"><a class="url fn n" href="' . esc_url( get_author_posts_url( get_the_author_meta( 'ID' ) ) ) . '">'. '%1$s' . get_the_author() . '</a></span>',
		     esc_html_x( 'by ', 'Used before post author name.', 'towy' )
		);
	}

endif; //towy_posted_by
...
if ( ! function_exists( 'towy_post_thumbnail' ) ) :
	function towy_post_thumbnail( $small_image = false ) {
		$pID = get_the_ID();
		...
		if ( get_post_format( $pID ) == 'gallery' ) {
			...
			towy_shortcode_atts_gallery_trigger( false );
		...
		<div
				...
				<div class="entry-meta-corner">
					...
					if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) :
						...
						<span class="comments-link">
							...
							<?php comments_popup_link( esc_html__( '0', 'towy' ), esc_html__( '1', 'towy' ), esc_html__( '%', 'towy' ) ); ?>
						</span>
						<?php
			...
			if ( ! $is_gallery ) :
				...
				if (
					...
					the_post_thumbnail( 'towy-full-width' );
				} elseif ( ! is_singular() && $small_image ) {
					the_post_thumbnail( 'towy-small-width' );
				} else {
					the_post_thumbnail();
				} //$current_position
			...
			// gallery
				...
				<div id="owl-carousel-<?php echo esc_attr( $pID ); ?>" class="owl-carousel"
				     ...
				     data-items="1"
				     data-autoplay="true"
                     data-speed="<?php echo function_exists( 'fw_get_db_customizer_option' )
                         ? esc_attr( ( (int) fw_get_db_customizer_option( 'slide_speed' ) ) * 1000 ) : 5000; ?>"
...
// share buttons
if ( ! function_exists( 'towy_share_this' ) ) :
	/**
	 * Share article through social networks.
	 * bool $only_buttons
	 */
	function towy_share_this( $only_buttons = false ) {
...
if ( ! function_exists( 'towy_get_predefined_template_part' ) ) :
	/**
	 * Return propper template part from options or default.
	 * string $template_part_name
	 */
	function towy_get_predefined_template_part( $template_part_name ) {
		...
		if ( function_exists( 'fw_get_db_customizer_option' ) ) {
			...
			$template_part = $template_part_name . '-3';
		}
	...
	} //towy_get_predefined_template_part()

endif;//function_exists

//get ids of showing widgets
if ( ! function_exists( 'towy_get_showing_widgets_ids' ) ) :
	/**
	 * Return array of id's of all widgets that are showing.
	 */

	function towy_get_showing_widgets_ids() {
...
if ( ! function_exists( 'towy_get_posts_single_taxonomy_name' ) ) :
	function towy_get_posts_single_taxonomy_name( $queried_object ) {
...
if ( ! function_exists( 'towy_get_post_categories' ) ) :
	function towy_get_post_categories( $taxonomy_name = 'category' ) {
...
if ( ! function_exists( 'towy_get_categories_slugs_for_single_post' ) ) :
	function towy_get_categories_slugs_for_single_post( $taxonomy_name = 'category' ) {
...
if ( ! function_exists( 'towy_is_shop' ) ) :
    function towy_is_shop() {
        $res = false;
...
// check if is WooCommerce Products Page. Need for Customizer setting.
if ( ! function_exists( 'towy_is_woocommerce_page' ) ) :
    function towy_is_woocommerce_page() {
        $res = false;
        if ( class_exists( 'WooCommerce' ) ) {
...
function towy_get_option( $name, $default = '' ) {
	//for demo
	if( ! empty( $_GET[$name] ) ) {
	...
	if ( function_exists( 'fw_get_db_customizer_option' ) ) {
		return fw_get_db_customizer_option( $name );
	} else {
		return $default;
	}
}